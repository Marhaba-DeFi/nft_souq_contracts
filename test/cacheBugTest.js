...
describe("Cache bug test", async () => {
	...
	before(async () => {
		...
		let { souqNFTDiamond, diamondCutFacet, diamondLoupeFacet } = await deployDiamond();
	...
	it("should not exhibit the cache bug", async () => {
		...
		const selectors = await global.diamondLoupeFacet.facetFunctionSelectors(
			global.test1Facet.address
		);
